	...
	<form method="get" action="confirmChange.html">
		<table border="1">
			...
			<tr>
				...
				<th>
					<select name="sex">
						<option value='m' <c:if test="${fn:contains(member.SEX, 'm')}">selected</c:if>>男性</option>
						<option value='f' <c:if test="${fn:contains(member.SEX, 'f')}">selected</c:if>>女性</option>
					</select>
				</th>
				<th>
					<input type="text" id="zip" name="zip" value="${member.ZIP}">
				</th>
				<th>
					<textarea id="address" name="address" rows="4" style="width: 100%; border: 0; resize: none;">${member.ADDRESS}</textarea>
				</th>
				<th>
